...
assetdir <-
  "D:/Code Repositories/FluHospPrediction/results/00_paper_output/"

# table font
global_table_font <- "Times New Roman"

# function to get a set of files based on their creation date
global_date <- "2020-11-12"     # set accordingly
...
rttab_cap <- function(target, analysis, component = FALSE) {
  ...
  paste0(
    ...
    paste0("Cross-validated risks for the ensemble and discrete super learner predictions of ", texts[t], ", by week of influenza season (", analysis, " analysis). Estimates presented as mean (standard error).")
  }
}
  ...
  paste(
    ...
    ifelse(
      ...
      paste0(
        ...
        texts[3], " (cumulative hospitalizations).")
    )
  )
}
...
supp <- data.table(
  ...
  cap = c(
    ...
    "Ensemble, component learner, and naive mean prediction risks by week of simulated flu season and prediction target (squared error sensitivity analysis). Learners assigned zero weights by the metalearner are omitted.",

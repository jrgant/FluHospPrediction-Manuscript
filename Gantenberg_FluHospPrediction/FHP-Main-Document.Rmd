...
1. Assign each observation to a fold for use in cross-validation. Each weekly dataset contained 15,000 rows, one for each simulated influenza hospitalization curve. To account for the lack of independence between curves, we assigned all simulated curves based on the same empirical shape template to the same fold (e.g., all curves based on the trend filter fit to season 2004-05). We did not account for potential dependencies due to repeated sampling of the noise term, as we believed the season shape was the more consequential source of dependence. (The noise injected into the simulated curves was typically very small, and the season shape determined the number of peaks in a simulated curve.)

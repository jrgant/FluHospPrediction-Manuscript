...
rwsum_pr[, (rdcols) := lapply(.SD, roundit), .SDcols = rdcols][] %>%
  flextable(cwidth = c(0.51, rep((6 - 0.51) / 5, 5))) %>%
  bold(., part = "header") %>%
...
```{r supp-peakrate-risks-lse-cap, results = "asis"}
...
```{r supp-peakweek-risks-lse-cap, results = "asis"}
supp_cap(slug)
```
\begin{minipage}{0.5\textwidth}
...
```{r supp-cumhosp-risks-lse-cap, results = "asis"}
...
```{r supp-peakrate-risks-erf-cap, results = "asis"}
...
```{r supp-peakweek-risks-erf-cap, results = "asis"}
...
```{r supp-cumhosp-risks-erf-cap, results = "asis"}
...
cumhosp_risks %>%
  ...
  bold(., part = "header") %>%
  align(part = "all", align = "center") %>%
  #add_footer_lines(risktabfn("Cumulative rate")) %>%
  font(., fontname = global_table_font, part = "all")
```
\end{minipage}
\hspace{0.05\textwidth}
...
cumhosp_risks %>%
  ...
  bold(., part = "header") %>%
  align(part = "all", align = "center") %>%
  add_footer_lines(risktabfn("Cumulative rate", sqe = TRUE)) %>%
  font(., fontname = global_table_font, part = "all")
```
\newpage
...
```{r supp-simtemplate-rand, results = "asis", out.width = "90%", fig.asp = 1.25}
slug <- "Simulation-Curves-by-Template-Rand10"
supp_cap(slug)
...
```{r fig.id="ensemble-perf-sqe", results = "asis", fig.width = 6, fig.asp = 4000/4000}
...
```{r supp-prosp-error-compare, results = "asis", fig.asp = 10800/32400}
slug <- "Prospective-Observed-Error"
supp_cap(slug)
knitr::include_graphics(get_asset("FIG", slug))
```
...
```{r supp-obs-on-sim, results = "asis", fig.asp = 10800/32400}
slug <- "Observed-Predict-Simulated"
supp_cap(slug)
knitr::include_graphics(get_asset("FIG", slug))
```

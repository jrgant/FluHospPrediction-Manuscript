...
For the peak rate and cumulative hospitalization rate prediction targets, the ensemble super learners trained on simulated and observed data both performed similarly to or improved upon on a prediction using the naive median from prior seasons across most weeks of the influenza season (Figure  \@ref(fig:prosp-application), Web Figure 17). These improvements were more apparent in the 2017--18 influenza season, which was more severe than 2016--17 and 2018--19 with respect to these outcomes. However, the ensembles trained only on observed data generated several predictions of much poorer quality than the ensemble trained in simulated data and, in some cases, even the empirically trained discrete super learner. The most notable instances occurred during the 2017--18 season---in Weeks 20 and 25 for peak rate and in Weeks 5 and 15 for peak week. Ensembles trained on simulated data generally produced predictions of more stable quality. An exception to this pattern arises for the cumulative hospitalization rate, where the ensemble super learner appears to generate less accurate and less stable predictions during the early weeks of the season, compared to the empirically trained ensembles. Neither type of ensemble appeared to improve predictions of the peak week, and in many cases, underperformed versus the naive median prediction for this prediction target.

As in the main analysis, the ensemble's prediction error from the super learner trained in simulated data usually was similar to that of the discrete super learner (with some exceptions), though the model selected as the discrete super learner varied by prediction target, season, and week.
...
Fourth, even the ensemble super learner may be subject to instability. In Web Figure 19, we describe an instance where the ensemble's cross-validated prediction risk in Week 16 for the peak hospitalization rate (main analysis) spiked to more than double that shown in Figures \@ref(fig:ensemble-perf-main) and \@ref(fig:ensemble-perf-regscale). We traced this spike to an apparently anomalous training run in a single validation set. Researchers should thoroughly diagnose the performance of ensemble models just as they would a statistical prediction model.
...
targets %>%
  ...
  set_caption(
    as_paragraph(
      "Empirical distributions of peak hospitalization rate, peak hospitalization week, and cumulative hospitalization rate in the United States, 2003--2019.", as_sup("a")
    )
  ) %>%
  compose(.,
...
^a^ Source: CDC FluView, Emerging Infections Program [@Centers_for_Disease_Control_and_Prevention2016-nr]. Pandemic influenza season 2009-2010 omitted.
...
<!-- (ref:tab3cap) Prediction risks for the ensemble and discrete super learner predictions of the peak hospitalization rate, peak hospitalization week, and cumulative hospitalization rate, by week of influenza season.^a^ Estimates presented as mean risk (standard error). -->

```{r table3, tab.id="main-analysis-risk-table"}
...
prwc_risks %>%
  ...
  set_caption(
    as_paragraph(
      "Prediction risks for the ensemble and discrete super learner predictions of the peak hospitalization rate, peak hospitalization week, and cumulative hospitalization rate, by week of influenza season.", as_sup("a"), " Estimates presented as mean risk (standard error)."
    ),
...
\newpage
```{r fig.id="prosp-application", fig.cap = "**Prospective assessment of super learners trained on simulated data and observed data, as well as a naive prediction based on observed data (median outcome from preceding seasons).** Predictions from ensemble and discrete super learners are shown in each week for both super learner routines (i.e., trained on observed vs. simulated data). Horizontal black dashed lines depict the prediction target, while horizontal gray dashed lines depict the median of a given outcome in preceding seasons. Peak hospitalization rate and cumulative hospitalizations per 100,000 population. A) Peak rate, 2016--17 influenza season; B) Peak rate, 2017--18 influenza season; C) Peak rate, 2018--19 influenza season; D) Peak week, 2016--17 influenza season; E) Peak week, 2017--18 influenza season; F) Peak week, 2018--19 influenza season; G) Cumulative hospitalizations, 2016--17 influenza season; H) Cumulative hospitalizations, 2017--18 influenza season; I) Cumulative hospitalizations, 2018--19 influenza season.", fig.width = 5.5, fig.asp = 14400/14400}
